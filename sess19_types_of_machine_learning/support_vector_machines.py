...
# an apple or an orange based on its weight and size

# Import the required modules
import matplotlib.pyplot as plt
import numpy as np
# from pandas.tests.frame.methods import test_reorder_levels
...
label_apples = np.zeros(50) # use label 0 for apples
...
label_oranges = np.ones(50) # use label 1 for oranges
...
def plot_decision_boundaries(X,y,model):
    ...
    xx, yy = np.meshgrid( # Generate 100 evenly spaced points btwn start & end
        np.linspace(xlim[0], xlim[1], 100), np.linspace(ylim[0], ylim[1], 100)
    )
    Z = model.decision_function(np.c_[xx.ravel(), yy.ravel()]) # Flatten the 2-D array into 1-D array
